    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        for piece in pieces:
            ...
            if len(candidate) <= self.chunk_size:
                ...
                continue
            if pending:
                split_chunks.extend(self._split(pending, next_separators))
            pending = piece
        if pending:
            split_chunks.extend(self._split(pending, next_separators))
        ...
        for chunk in split_chunks:
            if merged and len(merged[-1]) + len(separator) + len(chunk) <= self.chunk_size:
                merged[-1] = merged[-1] + separator + chunk
            else:
                merged.append(chunk)
        return merged
...
class HeadingChunker:
    """Split Markdown into heading-led sections, falling back for long sections."""
